...
import { ManageMusicVideoComponent } from "./manage-music-video/manage-music-video.component";
import { ViewVideoDailogComponent } from "./view-video-dailog/view-video-dailog.component";


@Component({
  selector: 'CoreLab-music-video',
...
export class MusicVideoComponent {
  ...
  openCreateDialog() {
    const dialogRef = this.dialog.open(ManageMusicVideoComponent, {
      data: { action: "CREATE", title: "Video Management", payload: null },
      width: "600px",
    });
  ...
  openUpdateDialog(row: Category) {
    const dialogRef = this.dialog.open(ManageMusicVideoComponent, {
      data: { action: "UPDATE", title: "Video Management", payload: row },
      width: "600px",
    });
  ...
  const fileData: any = {
    title: row?.videoMetadata?.caption,
    fileUrl: this.baseUrl+row?.url,
    fileType: 'video'
  };
    this.dialog.open(ViewVideoDailogComponent, {
      data: fileData,
      width: '600px',
